...
from Models.GNN.layer import (
    multi_shallow_embedding,
    Dense_TimeDiffPool2d,
    DenseGCNConv2d,
    DenseGINConv2d,
    DyGINConv2d,
    TimeDiffPool2d,
...
class GNNTest(nn.Module):
    ...
    def __init__(
        ...
        self.cate_encoder = CateEncoder(64)
        self.nume_encoder = nn.Conv1d(10, 64, 1, bias=False)
        self.demo_encoder = nn.Linear(4, 64, False)
        ...
        assert num_layers == len(
            kern_size
        ), "Error: Number of kernel_size should equal to number of layers."
        ...
        self.gconvs = nn.ModuleList(
            ...
            + [
                gnn_model(hidden_dim, heads * hidden_dim, groups)
        ...
        #     [
        #         Dense_TimeDiffPool2d(
        #             self.left_num_nodes[layer],
        #             self.left_num_nodes[layer + 1],
        #             kern_size[layer],
        #             paddings[layer],
        #         )
        #         for layer in range(num_layers - 1)
        ...
        #     + [
        #         Dense_TimeDiffPool2d(
        #             self.left_num_nodes[-2],
        #             self.left_num_nodes[-1],
        #             kern_size[-1],
        #             paddings[-1],
        #         )
        #     ]
    ...
    def reset_parameters(self):
        for tconv, gconv, bn, pool in zip(
            self.tconvs, self.gconvs, self.bns, self.diffpool
        ):
    ...
    def forward(self, cate, nume, demo):
        ...
        demo = demo.unsqueeze(2)
        demo_repeated = demo.repeat(1, 1, 24)
        h = torch.cat((cate, nume, demo_repeated), dim=1)
        # h = x_cate + x_nume + x_demo
        # inputs = h.permute(0, 2, 1).unsqueeze(1)
        inputs = h.unsqueeze(1)
        if inputs.size(-1) % self.num_graphs:
            pad_size = (self.num_graphs - inputs.size(-1) % self.num_graphs) / 2
            x = F.pad(
                inputs, (int(pad_size), ceil(pad_size)), mode="constant", value=0.0
            )
        else:
            x = inputs
