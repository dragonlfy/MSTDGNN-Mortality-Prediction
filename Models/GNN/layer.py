...
class Group_Linear(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, groups=1, bias=False):
        ...
        self.group_mlp = nn.Conv2d(in_channels * groups, out_channels * groups, kernel_size=(3, 1), padding=1, groups=groups, bias=bias)
...
class DenseGCNConv2d(nn.Module):
    ...
    def forward(self, x: Tensor, adj: Tensor, add_loop=True):
        ...
        if self.bias is not None:
            out = out.transpose(2, 3) + self.bias
...
class TimeDiffPool2d(nn.Module):
    ...
    def __init__(self, pre_nodes, pooled_nodes, kern_size, padding):
        ...
        self.re_param = Parameter(Tensor(pooled_nodes, 1))
    ...
    def forward(self, x: Tensor, adj: Tensor):
        ...
        out = self.time_conv(x)
        out = out.transpose(1, 2)
        a_transpose = x.permute(0, 2, 3, 1)
        b_transpose = out.permute(0, 1, 3, 2)
        a = a_transpose[0, 0, 0, :]
        b = b_transpose[0, 0, 0, :]
        weight = torch.outer(b, a).unsqueeze(2).unsqueeze(3)
        final_weight = weight.repeat(1, 1, 1, b.size(-1))
        # s: [ N^(l+1), N^l, 1, K ]
        s = torch.matmul(final_weight, self.re_param).view(out.size(-2), -1)
