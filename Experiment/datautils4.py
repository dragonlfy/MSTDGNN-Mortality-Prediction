...
class SegDataset(Dataset):
    def __init__(self, seg_list, split, window) -> None:
        ...
        self.labels = mort_u_arr.reshape((-1, 1))

        print(f"{split:5s} {window} | {len(self):5d}",
    ...
    def relalel(self, predicted, weight):
        ...
        self.labels = self.labels * (1.0 - weight)+predicted * weight
...
class DLoader(DataLoader):
    def __init__(self, segs_list, batch_size, shuffle, num_workers,
                 split, window):
...
def load_data(fpath, n_fold, i_fold, num_groups):
    ...
    train_sll = [segs_list[idx] for idx in np.concatenate([dataset.indices for dataset in datasets]).tolist()]
...
class MIMIC4DataModule:
    ...
    def get_dataloaders(self, window, isolate=False):
        ...
        train_loader = DLoader(train_segs, batch_size, True, num_workers,
                               'Train', window)
        valid_loader = DLoader(val_segs, batch_size, False, num_workers,
                               'Val', window)
        test_loader = DLoader(test_segs, batch_size, False, num_workers,
                              'Test', window)

        if 0 <= window < self.cfg.num_groups - 1:
            nextwind = self.train_segs_grp[window+1: window+2]
            nextwind_loader = DLoader(nextwind, batch_size, False,
                                      num_workers, 'Next', window)
        else:
    ...
    def get_datasets(self, window):
        ...
        segdatasets = [loader.segdataset if loader is not None else None for loader in loaders]
        datasets = []
...
def test():
    ...
    cfg = ExperimentConfig('mimic4', 'DPGap4SPGap48Len24Str12Art2Grp8')
    data_module = MIMIC4DataModule(cfg)
